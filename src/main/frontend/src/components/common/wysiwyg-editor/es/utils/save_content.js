...
var SaveBehavior = function () {
  ...
  SaveBehavior.prototype.store = function store(content, title, subtitle, saveType) {
    var _this = this;
    if (this.getLocks() > 0) {
      // console.log('locked')
      return;
    }
    return _this.checkforStore(content, title, subtitle, saveType);
  };
  ...
  SaveBehavior.prototype.checkforStore = function checkforStore(content, title, subtitle, saveType) {
    ...
    // console.log("CONTENT CHANGED:", isChanged)
      this.save(content, saveType, title, subtitle);
    
  };

  SaveBehavior.prototype.save = function save(content, title, subtitle, saveType) {
    var _this2 = this;
    // use save handler from config if exists
    if (this.config.data_storage.save_handler) {
        this.config.data_storage.save_handler(_JSON$stringify(content), title, subtitle, saveType);
        return;
    }
