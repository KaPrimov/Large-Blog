...
import {
  convertToRaw, convertFromRaw, CompositeDecorator,
  //getVisibleSelectionRect, 
  getDefaultKeyBinding,
  //getSelectionOffsetKeyForNode, 
  //KeyBindingUtil, 
  ContentState, Editor, EditorState, Entity, RichUtils, DefaultDraftBlockRenderMap, SelectionState, Modifier
} from
  //BlockMapBuilder, 
  //getSafeBodyFromHTML 
  'draft-js';
...
import {
  addNewBlock, resetBlockWithType, updateDataOfBlock,
  //updateTextOfBlock, 
  getCurrentBlock, addNewBlockAt
} from '../model/index.js';
...
var DanteEditor = function (_React$Component) {
  ...
  function DanteEditor(props) {
    ...
    _this.state = {
      ...
      body_placeholder: _this.props.config.body_placeholder,
    ...
    _this.save = new SaveBehavior({
      ...
      title: _this.props.titleValue,
      subtitle: _this.props.subtitleValue,
    });
    return _this;
  }
  ...
  DanteEditor.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
    var _this13 = this;
    if (nextProps.isForSave) {
      return _this13.save.store(_this13.emitSerializedOutput(), _this13.props.titleValue, _this13.props.subtitleValue, nextProps.saveType);
    }
  ...
  DanteEditor.prototype.onChange = function onChange(editorState) {
    var _this2 = this;
    this.setPreContent();
    //placeholder disapears on click
    ...
    if (this.props.isForSave) {
      // return this.dispatchChangesToSave(); uncomment to save on change
    }
  };

  DanteEditor.prototype.dispatchChangesToSave = function dispatchChangesToSave(type) {
    var _this3 = this;
    return _this3.save.store(_this3.emitSerializedOutput(), _this3.props.titleValue, _this3.props.subtitleValue, type);
  };
    ...
    if (!e.altKey && !e.metaKey && !e.ctrlKey) {
      ...
      if (currentBlock.getText().length > 0) {
        ...
        if (blockType === "unstyled") {
          ...
          var newContent = Modifier.splitBlock(this.state.editorState.getCurrentContent(), this.state.editorState.getSelection());

          var newEditorState = EditorState.push(this.state.editorState, newContent, 'insert-characters');
  ...
  DanteEditor.prototype.handleKeyCommand = function handleKeyCommand(command) {
    ...
    if (newState) {
      if (block.getType() === 'image') {
        this.props.deleteHandler(block.getData());
      }
  ...
  DanteEditor.prototype.relocateTooltips = function relocateTooltips(e) {
    var _this10 = this;
  ...
  DanteEditor.prototype.tooltipsWithProp = function tooltipsWithProp(prop) {
    if (this.state.editorState.getCurrentContent().getFirstBlock()['type'] !== 'unstyled') {
      this.setState({body_placeholder: ''});
    }
  ...
  DanteEditor.prototype.hideTooltip = function hideTooltip() {
    jQuery('.inlineTooltip-button').hide();
  }

  DanteEditor.prototype.showTooltip = function showTooltip() {
    jQuery('.inlineTooltip-button').show();
  }
  ...
  DanteEditor.prototype.render = function render() {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        !this.props.config.read_only && 
        React.createElement(
          'div',
          {className: 'create-news-toolbar'},
          React.createElement(
            'div',
            {className: 'tools'},
            (!this.props.config.read_only && !this.props.isMetadataButtonHidden) &&
            React.createElement(
              'button',
              {className: 'btn btn-sm btn-ocustom standart-button-workflow', onClick:() => _this12.dispatchChangesToSave('metadata')},
              <Translate value="wysiwyg_editor_labels.button_to_metadata" />
            ),
            !this.props.config.read_only &&
            React.createElement(
              'span',
              ...
              React.createElement(
                'button',
                {onClick:() => _this12.dispatchChangesToSave('content'), className: 'btn btn-sm btn-ocustom' },
                <Translate value="wysiwyg_editor_labels.button_save_content"/>
              )
            )
          )
        ),
              ...
              React.createElement(
                ...
                (this.props.config.read_only ?
                  React.createElement('h1',
                    { className: 'dante-reader-h1' },
                    this.props.titleValue 
                  ) : 
                  React.createElement('div',
                    { className: 'editor-h1-container section-inner layoutSingleColumn' },
                    React.createElement('input', {type:'text', className: 'editor-h1-title', placeholder: I18n.t('wysiwyg_editor_labels.title'), name: 'title', onChange: this.props.onChangeContent, value: this.props.titleValue, disabled: this.props.config.read_only, onFocus: this.hideTooltip, onBlur: this.showTooltip })
                  )
                ),
                this.props.config.show_headings &&
                (this.props.config.read_only ?
                  React.createElement('h3',
                  { className: 'dante-reader-h3' },
                    this.props.subtitleValue 
                  ) : 
                  React.createElement('div',
                    { className: 'editor-h3-container section-inner layoutSingleColumn' },
                    React.createElement('input', {type:'text', className: 'editor-h3-title', placeholder: I18n.t('wysiwyg_editor_labels.subtitle'), name: 'subtitle', onChange: this.props.onChangeContent, value: this.props.subtitleValue, disabled: this.props.config.read_only, onFocus: this.hideTooltip, onBlur: this.showTooltip })
                  )
                ),                
                (this.props.config.read_only && this.props.showImage) &&
                React.createElement('div',
                {className: 'article-image-container'},
                  React.createElement('img', 
                  {className: 'article-image-container', src: this.props.imagePath} )),
                React.createElement(
                  'div',
                ...
                React.createElement(
                  ...
                  React.createElement(
                    ...
                    React.createElement(Editor, {
                      ...
                      placeholder: this.state.body_placeholder,
                      ref: 'editor'
                    })
                  )
                )
