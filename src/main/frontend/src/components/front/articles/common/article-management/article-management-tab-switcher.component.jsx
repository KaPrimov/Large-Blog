...
const ArticleManagementTabSwitcher = ({onSwitchBetweenTabs, activeTab}) => {
	return (
		<div className="article-management-buttons row">
			<button className={'btn btn-sm btn-ocustom col-sm-12 col-md-2 offset-md-3 management-button' + (activeTab === ArticleStatus.DRAFT? ' standart-button-workflow' : '') }
				onClick={() => onSwitchBetweenTabs(ArticleStatus.DRAFT)}>
				<Translate value="article_management.draft" />
			</button>
			<button className={'btn btn-sm btn-ocustom col-sm-12 col-md-2 offset-md-3 management-button' + (activeTab === ArticleStatus.PUBLISHED ? ' standart-button-workflow' : '') }
